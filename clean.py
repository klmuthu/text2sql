...
try:
    ...
    rds.delete_db_subnet_group(DBSubnetGroupName=config['resources']['subnet_group_name'])
...
try:
    ...
    for secret in secrets_list['SecretList']:
        ...
        if (config['aurora']['cluster_identifier'] in secret['Name'] and 
            not secret['Name'].startswith('rds!')):
            try:
        ...
        for sg in sgs:
            if sg['GroupName'] != 'default':
                try:
                    ...
                    print(f"Error removing rules from security group {sg['GroupId']}: {e}")
